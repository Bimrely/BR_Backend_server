...
export const commentArticle = async (req, res) => {
  try {
    ...
    // Get the article owner ID
    const articleOwnerId = article.userId.toString();

    // Send notifications to mentioned users
    for (const mentionedProfile of mentionedProfiles) {
      const mentionedUserId = mentionedProfile.userId;

      // Send mention notification
      const mentionNotification = new Notification({
        user: mentionedUserId,  // The mentioned user's ID from Profile
        type: 'mention',
        article: article._id,
    ...
    // Send comment notification to the article owner only if they are not already mentioned
    if (articleOwnerId !== userId.toString() && !mentionedIds.includes(articleOwnerId)) {
      const articleOwnerNotification = new Notification({
        user: articleOwnerId,  // The article owner
        type: 'comment',
        article: article._id,
